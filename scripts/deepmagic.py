...
MODEL_NAME = "rombodawg/DeepMagic-Coder-7b-Alt"
...
def load_model_and_tokenizer():
    """Load model with FP16 support for AMD ROCm."""
    logger.info(f"Loading model {MODEL_NAME} with FP16 precision...")
    model_path = f"{MODEL_DIR}/{MODEL_NAME.replace('/', '_')}"
    device = check_gpu()
    try:
        model = AutoModelForCausalLM.from_pretrained(
            MODEL_NAME,
            torch_dtype=torch.float16,  # Use FP16 for ROCm compatibility
